...
const ADOPTION_APP_NAV_REFERENCE = [
  ...
  '',
  '**Settings** (sidebar): **OpenAI API key** for the assistant (pack KV `openaiKey` and proxies when deployed; local dev may store in the browser — see on-screen help); **After adding a source** (ask every time vs always guided wizard vs always manual form); **Plan dashboard prompts** (show or hide the “Plan in shape? Activate it.” Activation nudge when no tier is set); **Detail page card expansion** defaults on source and worker-group detail pages; **UI animations** toggle; **Reset Plan** wipes the current plan after confirmation; **About this build** (version string); **Feedback & app support** (email **mmolina@cribl.io** for app issues or feedback).',
  '',
